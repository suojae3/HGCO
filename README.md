...
26. 지정방식에 따른 오퍼랜드 필드에 명시하는 값을 정리하자면 아래와 같습니다
- 즉시 주소 지정 방식: 연산에 사용할 데이터
- 간접 주소 지정 방식: 유효 주소(메모리 주소)
...
### [Chapter 04] CPU의 작동원리 <br/>

1. CPU 내부에는 계산을 담당하는 ALU, 명령어를 읽어 들이고 해석하는 제어장치, 작은 임시 저장 장치인 레지스터라는 구성 요소가 있습니다.
2. 1+2를 계산할 때 1과 2라는 피연산자와 더하기라는 수행할 연산이 필요하듯 ALU가 계산하기 위해서는 피연산자와 수행할 연산이 필요합니다.
3. ALU는 레지스터를 통해 피연산자를 받아들이고, 제어장치로부터 수행할 연산을 알려주는 제어신호를 받아들입니다. 이를 통해 산술 연산, 논리 연산 등 다양한 연산을 수행합니다.
4. 연산을 수행한 결과는 특정 문자이거나 메모리 주소가 될 수도 있습니다. 이러한 결과값은 바로 메모리에 저장되는 것이 아닌 일시적으로 레지스터에 저장됩니다.
5. CPU가 메모리에 접근하는 속도는 레지스터에 접근하는 속도보다 훨씬 느립니다.
6. ALU는 계산결과와 더불어 **플래그**를 내보냅니다. 플래그는 CPU가 프로그램 실행 도중 반드시 기억해야하는 일종의 참고정보입니다.
7. 대표적인 플래그로는 부호 플래그, 제로 플래그, 캐리 플래그, 오버플로우 플래그, 인터럽트 플래그, 슈퍼바이저 플래그가 있습니다.
8. CPU를 구성하는 요소 중 두번째, 제어장치는 제어신호를 내보내고 명령어를 해석하는 부품입니다.
9. 제어장치는 먼저 클럭신호를 받아들입니다. 이때 클럭(Clock)이란 컴퓨터의 모든 무품을 일사분란하게 움직일 수 있게 하는 시간 단위입니다.
- 클럭의 똑딱똑딱 주기에 맞춰 레지스터에서 다른 레지스터로 데이터가 이동되거나 ALU에서 연산이 수행되거나 CPU가 메모리에 저장된 명령어를 읽습니다.
10. 클럭신호를 받은 제어장치는 다음으로 '해석해야할 명령어'를 받습니다.
- 이때 해석할 명령어는 명령어 레지스터라는 특별한 레지스터로부터 받아들이고 해석한 뒤 제어 신호를 발생시켜 컴퓨터 부품들에게 수행해야할 내용을 알려줍니다.  
12. 해석할 명령어를 받은 후 추가적으로 제어장치는 플래그 값을 플래그 레지스터로부터 받습니다. 플래그를 참고하여 제어신호를 발생시킵니다.
13. 제어장치는 시스템 버스, 그중에서 제어 버스로 전달된 제어 신호를 받습니다.
- 제어 신호는 CPU뿐만 아니라 외부 입출력 장치에서도 발생시킬 수 있습니다. 따라서 제어장치는 제어버스를 통해 외부 제어 신호를 받기도 합니다.
14. 제어장치는 CPU내부와 외부 모두 제어신호를 내보낼 수 있습니다.
15. 외부로 보내는 제어신호에는 크게 두가지, 메모리에 전달하는 제어신호와 입출력 장치에 전달하는 제어신호가 있습니다.
16. 내부로 보내는 제어신호로는 크게 두가지, ALU에 전달하는 제어신호(연산수행)와 레지스터에 전달하는 제어신호(레지스터간 데이터 이동하거나 해석)가 있습니다.
17. 프로그램 속 명령어와 데이터는 실행전후로 반드시 레지스터에 저장됩니다.
18. 반드시 알아야할 레지스터의 종류로는 아래 8가지가 있습니다.
- 프로그램 카운터: 메모리에서 가져올 명령어의 주소를 저장합니다. 명령어 포인터라 불리기도 합니다.
- 명령어 레지스터: 해석할 명령어 즉 방금 메모리에서 읽어들인 명령어를 저장하는 레지스터 입니다.
- 메모리 주소 레지스터
- 메모리 버퍼 레지스터
- 플래그 레지스터
- 범용 레지스터
